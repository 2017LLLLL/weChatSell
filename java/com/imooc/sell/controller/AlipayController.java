...
import com.imooc.sell.comfigura.AlipayConfig;
...
public class AlipayController {
	...
	public String goAlipay(String orderId, HttpServletRequest request, HttpServletRequest response) throws Exception {
		...
		OrderDetail orderDetail = order.getOrderMasterList().get(0);
		...
		//付款金额，必填
		String total_amount = order.getOrderAmount().toString();
		//订单名称，必填
		String subject = orderDetail.getProductName();
		//商品描述，可空
		String body = "number:" + orderDetail.getProductQuantity();
...
*/
			...
			/*修改支付状态*/
			OrderDto order = orderService.findOne(out_trade_no);
			OrderDetail orderDetail = order.getOrderMasterList().get(0);
			Flow flow = new Flow();
			flow.setBuyCounts(orderDetail.getProductQuantity());
			flow.setFlowNum(trade_no);
			flow.setId(KeyUtils.getUniqueKey());
			flow.setProductId(orderDetail.getProductId());
			flow.setPaidMethod(1);
			flow.setOrderNum(out_trade_no);
			flow.setPaidAmount(order.getOrderAmount().toString());
			orderService.pay(order,flow);
    		...
    		log.info("* 购买产品: {}", orderDetail.getProductName());
    		log.info("***************************************************************");
			...
			mv.addObject("product", order.getOrderMasterList());
